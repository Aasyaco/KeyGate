...
app.post("/api/generate-token", globalLimiter, authenticateRequest, apiKeyLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!validation.success) return sendError(res, 400, ErrorCode.VALIDATION_ERROR, "Protocol validation failed.", traceId);
...
// --- VERCEL / PROD STATIC SERVING FALLBACK ---
...
startBootstrap().catch(console.error);
